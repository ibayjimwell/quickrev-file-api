...
async def update_file_content_endpoint(payload: FileUpdatePayload) -> Dict[str, Any]:
    file_id = payload.file_id
    new_content_bytes = payload.content.encode('utf-8')
    
    try:
        file_wrapper = InputFile.from_bytes(
            new_content_bytes, 
            filename=f"{file_id}.md"
        )
    except Exception as e:
        print(f"INPUTFILE INSTANTIATION FAILURE: {type(e).__name__}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"success": False, "message": "Failed to prepare file data for upload."}
        )
    
    try:
        # 2. UPDATE/REPLACE the file content in Appwrite Storage
        # FIX: Reverting to create_file, but adding the required positional permission arguments
        # which MUST be passed in this old SDK version, otherwise the API call fails or conflicts.
        
        # Positional arguments for create_file in appwrite==13.4.1:
        # 1. bucket_id
        # 2. file_id (Existing ID for overwrite)
        # 3. file_content_wrapper
        # 4. read_permissions
        # 5. write_permissions
        
        replace_result = cloud_storage.create_file(
            APPWRITE_BUCKET_ID,     # Arg 1: bucket_id
            file_id,                # Arg 2: file_id (to overwrite)
            file_wrapper,           # Arg 3: File content
            ['role:all'],           # Arg 4: Read Permissions (Required for the positional slot)
            ['role:member']         # Arg 5: Write Permissions (Required for the positional slot)
        )

        # 3. Return Success
        return {
            "success": True, 
            "message": "File content updated successfully via file replacement.",
            "file_id": replace_result['$id']
        }

    except AppwriteException as e:
        error_detail = f"Appwrite API Call Failed during file replacement. Status: {e.code}. Message: {e.message}."
        print(f"APPWRITE FAILURE: {error_detail}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"success": False, "message": f"Appwrite error: {e.message}"}
        )
    except Exception as e:
        print(f"GENERAL UPDATE FAILURE: {type(e).__name__}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"success": False, "message": f"Unexpected server error: {str(e)}"}
        )